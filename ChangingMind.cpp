...
//    fout_learning_unique_paper << "Label" << endl;
	...
	cerr << " done with proof" << endl;
	solve(macro_to_num["\\Reals"]);
...
/*	*/
...
#if K_2K_DEF
	fout_k_2k_thresholds.open(folder + "k_2k_thresholds.txt");
	fout_k_2k_thresholds << "K Threshold InstanceCount" << endl;
    for(int i = 0; i < 20; i++) {
        fout_k_2k[i].open(folder +"k_2k_prediction_" + to_string(10 * (i+1))  + ".txt");
        ...
        fout_k_2k[i] << "NumberOfDollarSigns, NumberOfBackSlashes, MaxCurlyBracesDepth, HalfSpeed, FullSpeed, ";
    ...
    for(int i = 0 ; i < 20; i++) {
        ...
        fout_k_2k_thresholds << 10 * (i + 1) << " " << threshold << " " << k_2k[i].size() << endl;
    }
#endif
