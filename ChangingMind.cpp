...
//  Most authoratitive author prediction
    for(int i = 0; i < (int)word_bucket[x].size(); i++) {
        ...
        if(check == true && count_equal_names == 1 && bound == N_TO_1 && (null_names == 0 || (null_names == 1 && SMART == "_smart" && (int)macro.authors.size() == N_TO_1))) {
            PreProcessGraphFeatures(x, word_bucket[x][i], local_author_count);
            ...
            if(same_date == false) {
                ...
                if(N_TO_1 == 2 && unique_authorPair.find(make_pair(macro.authors[0], macro.authors[1])) == unique_authorPair.end()) {
                    fout_learning_unique_authorPair << rev_macro_to_num[macro.macro_number] << ", " << winner << endl;  
                }
...
int main() {
    ...
    */
    ...
    for(int i = 1; i <= N_TO_1; i++) {
        ...
        fout_learning_unique_authorPair << "GlobalCurExp" + temp + ", LocalCurExp" + temp + ", LocalFracChange" + temp + ", IsMax" + temp +", IsMin" + temp + ", " + "IsMostRecent" + temp + ", " + "MacroNameLength" + temp + ", " + "CoAuthorCountUsedMacro" + temp + ", "; 
    }
